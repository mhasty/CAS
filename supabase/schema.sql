...
-- Functional order detail additions. Safe to run more than once.
alter table orders add column if not exists client_user text;
...
alter table orders add column if not exists product text default '1004 (UAD) - URAR';
alter table orders add column if not exists loan_type text;
alter table orders add column if not exists intended_use text;
alter table orders add column if not exists occupancy text;
alter table orders add column if not exists sale_price numeric(12,2);
alter table orders add column if not exists current_status text;
alter table orders add column if not exists date_needed text;
alter table orders add column if not exists inspection_scheduled_at text;
alter table orders add column if not exists appraiser_fee numeric(10,2);
...
drop policy if exists "order notes scoped select" on order_notes;
drop policy if exists "order notes scoped insert" on order_notes;
drop policy if exists "status history scoped select" on order_status_history;
drop policy if exists "status history scoped insert" on order_status_history;

create policy "order notes scoped select" on order_notes for select using (
  organization_id = current_profile_org()
  and (
    current_profile_role() in ('administrator','operations_manager','appraiser_manager','reviewer','accounting')
    or exists (select 1 from orders o where o.id = order_id and o.assigned_appraiser_id = auth.uid())
...
create policy "order notes scoped insert" on order_notes for insert with check (
  organization_id = current_profile_org()
  and current_profile_role() <> 'borrower_limited'
);
create policy "status history scoped select" on order_status_history for select using (
  organization_id = current_profile_org()
);
create policy "status history scoped insert" on order_status_history for insert with check (
  organization_id = current_profile_org()
);
